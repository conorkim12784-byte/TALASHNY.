    ...
    async def join_call(self, chat_id: int, original_chat_id: int, link, video: Union[bool, str] = None):
        ...
        try:
            ...
            try:
                ...
                raise AssistantErr("**- معليش ياعيني بس المكالمة مقفلة !\n\n- اذا كانت مفتوحه قفلها وارجع افتحها واذا طلعت هذي الرسالة اكثر من مرة ارسل امر /restart")
        except AlreadyJoinedError:
            raise AssistantErr("**معليش صار فيه خطا 😐 !\n\n- تطلع هذي المشكلة احيانا اذا شغلت مرتين ورا بعض ..\n\n- ارسل امر /restart وبتنحل ياعيوني")
        except TelegramServerError:
            raise AssistantErr("**- صار فيه خطا في خوادم Telegram And Mira ,\n\n- قفل المكالمة وارجع افتحها وشغل من جديد ولو طلعت لك الرسالة هذي مرة ثانية ارسل امر /restart")
    ...
    async def change_stream(self, client, chat_id):
        ...
        try:
            ...
            if "live_" in queued:
                ...
                stream = (
                    AudioVideoPiped(videoid, audio_parameters=audio_stream_quality, video_parameters=video_stream_quality)
                    if str(streamtype) == "video"
                ...
                stream = (
                    AudioVideoPiped(queued, audio_parameters=audio_stream_quality, video_parameters=video_stream_quality)
                    if str(streamtype) == "video"
