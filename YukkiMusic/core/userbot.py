...
class Userbot(Client):
    ...
    async def start(self):
        ...
        if config.STRING1:
            ...
            try:
                await self.one.join_chat("NvvvC")
                await self.one.join_chat("NvvvM")
                await self.one.join_chat("JuuuT")
            except:
                pass
            assistants.append(1)
            try:
                await self.one.send_message(
                    config.LOG_GROUP_ID, "-› **تم تشغيل الحساب المساعد بنجاح**"
                )
            except:
        ...
        if config.STRING2:
            ...
            try:
                await self.two.join_chat("NvvvC")
                await self.two.join_chat("Nvvvm")
                await self.two.join_chat("JuuuT")
            except:
                pass
            assistants.append(2)
            try:
                await self.two.send_message(
                    config.LOG_GROUP_ID, "-› **تم تشغيل الحساب المساعد الثاني بنجاح**"
                )
            except:
                LOGGER(__name__).error(
        ...
        if config.STRING3:
            ...
            try:
                await self.three.join_chat("NvvvM")
                await self.three.join_chat("NvvvC")
                await self.three.join_chat("JuuuT")
            except:
                pass
            assistants.append(3)
            try:
                await self.three.send_message(
                    config.LOG_GROUP_ID, "-› **تم تشغيل الحساب المساعد الثالث بنجاح**"
                )
            except:
                LOGGER(__name__).error(
