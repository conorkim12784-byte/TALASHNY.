...
if __name__ == "__main__":
    asyncio.run(main())    await q.edit_message_caption(caption=text, reply_markup=main_menu_keyboard())
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("شغل ") or text.startswith("تشغيل "):
        ...
        await update.message.reply_text(
            ...
            f"1️⃣ {query} - Official Music Video\n"
            f"2️⃣ {query} - Lyrics\n"
            f"3️⃣ {query} - Live Performance\n\n"
            f"⚡ Developer by {DEVELOPER}", parse_mode="Markdown"
        )
        ...
        for i in [25, 50, 75, 100]:
            ...
            bar = "█" * (i//10) + " " * (10 - i//10)
            try: await msg.edit_text(f"⬇️ جاري تحميل: {query}\n[{bar}] {i}%")
            except: pass
        await msg.edit_text(f"✅ تم تحميل: {query}\n\n⚡ Developer by {DEVELOPER}")
        ...
        for i in [25, 50, 75, 100]:
            await asyncio.sleep(1)
            bar = "█" * (i//10) + " " * (10 - i//10)
            try: await msg.edit_text(f"🎵 جاري تنزيل: {query}\n[{bar}] {i}%")
            except: pass
        await msg.edit_text(f"✅ تم تنزيل: {query} 🎵\n\n⚡ Developer by {DEVELOPER}")
